...
describe('MMU', () => {
  ...
  describe('ROM checks', () => {
    ...
    it('should detect if game is in colour', () => {
      mmu._rom[mmu.ADDR_IS_GB_COLOR] = mmu.IS_GB_COLOR;
      assert.equal(mmu.isGameInColor(), true);
      mmu._rom[mmu.ADDR_IS_GB_COLOR] = mmu.IS_GBC_ONLY;
      assert.equal(mmu.isGameInColor(), true);
      mmu._rom[mmu.ADDR_IS_GB_COLOR] = 0;
      assert.equal(mmu.isGameInColor(), false);
    });
